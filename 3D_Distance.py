...
# ----------------------------------------------------------------------------------
# ----------------------- Calculating Distance to Pellet ---------------------------
# ----------------------------------------------------------------------------------
...
point_indices = [] #see line 293

#this loop is to get a list of all the indices where there is a non-null datapoint, to get time values for plot later
for i in range(0,len(iso_reach_data[0])):
    if (math.isnan(iso_reach_data[0][i]) == False): #if it is not null, then the other axes are also not null
...
for i in range(0, len(iso_reach_data[0])):
    if (math.isnan(iso_reach_data[0][i]) == False):  #if it is not null, then the other axes are also not null
        d_X = iso_reach_data[0][i] - placeholder_pel_X
        d_Y = iso_reach_data[1][i] - placeholder_pel_Y
        d_Z = iso_reach_data[2][i] - placeholder_pel_Z
...
### !!!!!!!!!!!!!!!!!
### future to do: 
### draw a dot in space representing the pellet, since the pellet cannot be tracked with mocap  -> edit: done w placeholder values, but I'll keep my current report figures as is
### need to find a way to figure out where in 3d space the pellet is
### proposition: place a marker in the divot, and run a recording with optitrack tracking it.
### extract the trial data (1 trial is sufficient) and use these coordinate points for all analysis
### caveat it wont be where the real pellet is during actual trials, but it should be close enough
### then we won't miss out on being able to calculate things like distance to pellet, since if a mouse is reaching while the pellet is still moving
### and is able to grab the pellet as soon as it stops, it could show superior reach timing skills, which is also something Fmr1 KO mice have difficulty in
...
for i in range(0, len(point_indices)):
    temp = point_indices[i] / fps  #divide by the frame rate defined near the top of this document
    x.append(temp)
...
#customizing plot
plt.suptitle('Distance vs. Time', fontsize=25, fontfamily='fantasy')
plt.ylabel('Distance (cm)', fontsize=14, fontfamily='sans-serif', weight="bold") #add and customize y axis label
