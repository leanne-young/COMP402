#Trajectory Plot - 3D (OptiTrack Data)
#cumulative
...
#import csv
data = pd.read_csv (r"C:\Users\Iron Precision\Desktop\School\Lab_2022\S2022\COMP402\Python_Files\Opti_Reach_Simulation.csv")
...
for i in range(0, len(trial_data)): #to loop through the trials in the list   -> trial_data values are: [z, x, y] axes
    ...
    #partitioning of current trial complete, if a reach attempt exists during the trial, add it to the trials list
    if (reach_bool):
...
#SPECIFY VALUES FOR PLOTTING HERE
trial_number = 5
reach_number = 220
isolate_reach = True   #set this to false to plot all reaches in a trial

#plot all reaches in a trial
if (isolate_reach == False):
    timeaxis = list(range(0, trial_frames))
    #set axes ranges  -> these should be redefined for a real mouse trial, consider getting the global max and min values of the data and adding a buffer -> will do in the Fall
    y_small = 4.0
    y_big = 5.2
    x_small = 12
    x_big = 16
    z_small = 0
    z_big = 3
    #ax.set_ylim(4.0,5.2) #x
    #ax.set_xlim(12,16) #z
    #ax.set_zlim(0, 3) #y
    colormap = 'autumn'

#plot an isolated reach in a trial
if (isolate_reach == True):
    timeaxis = list(range(0, len(isolated_reaches_allTrials[trial_number][reach_number][0])))
    #y_small = 4.2
    #y_big = 4.8
    #x_small = 13.2
    #x_big = 15.7
...
#set axes ranges  -> these should be redefined for a real mouse trial, consider getting the global max and min values of the data and adding a buffer -> will do in the Fall
ax.set_ylim(y_small,y_big) #x
ax.set_xlim(x_small,x_big) #z
ax.set_zlim(z_small, z_big) #y
#plot cumulative datapoints from a whole trial:
#ax.scatter(trial_data[trial_number][0], trial_data[trial_number][1], trial_data[trial_number][2], c=timeaxis, cmap = 'autumn', s=5, alpha=0.5)

#plot data from an isolated reach in 1 trial
ax.scatter(isolated_reaches_allTrials[trial_number][reach_number][0], isolated_reaches_allTrials[trial_number][reach_number][1], isolated_reaches_allTrials[trial_number][reach_number][2], c=timeaxis, cmap = colormap, s=5, alpha=0.5)

ax.scatter(15.5, 4.5, 2, color = 'red', s=30, alpha=0.5) #plot an imaginary pellet

# ---------------------------------------------------------------------------------------------
# --------- graphing a line / meshgrid plane to indicate location of the slit in the wall -----
# --------- will revisit this during the fall semester and figure this out --------------------

#ax.xaxis.set_gridline_color((0, 'blue'))
...
#plt.plot([5, 6], [y_min, y_max], [z_min, z_max], 'k-', lw=2, color = 'blue')
#xx, yy = np.meshgrid(range(10), range(10))
#z = (100 - xx - yy) / 2 
#ax.plot_surface(xx, yy, z, alpha=0.5)

#ax = Axes3D(fig)
#ax.view_init(elev=0, 45)
...
if (POV == "side"):
      ax.view_init(elev=0, azim=90) #side view of trajectory (on LH side of mouse), eye level
if (POV == "top"):
      ax.view_init(elev=90, azim=180) #top view of trajectory (slit at the top, as viewed if standing behind the apparatus)
if (POV == "front"):
      ax.view_init(elev=0, azim=180) #front view, eye level
if (POV == "back"):
      ax.view_init(elev=0, azim=-180) #back view, eye level
if (POV == "angled"):
      ax.view_init(elev=25, azim=135) #angled view, viewing the apparatus from left back corner
...
#ax.grid(False)
#ax.set_axis_off()
#plt.title('Trajectory of Forepaw', fontsize=20, fontfamily='fantasy', y=1)
ax.set_xlabel('Z', fontsize=13, fontfamily='sans-serif', weight="bold") #customize x axis label
ax.set_ylabel('X', fontsize=13, fontfamily='sans-serif', weight="bold") #add and customize y axis label
ax.set_zlabel('Y', fontsize=13, fontfamily='sans-serif', weight="bold")
ax.get_yaxis().set_visible(False)

# Fall sem to do: find a way to hide the axis ticks that are all jumbled depending on viewpoint
# this is surpringly not straight forward for some reason in 3D graphsS
#ax.tick_params(
#    axis = 'y',
#    which = 'both',
#    bottom = False,
