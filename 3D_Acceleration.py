...
#acceleration uses velocity values, so it relies on the velocity component working. 
#Will need to double check if the velocity is all good before revisiting the acceperation to access if it is also good
#especially since this is a simulation (so there isn't previous data to compare it to, its juust how I moved the wand), and is not an intuitive value like distance
...
for i in range(0, len(velocities)-1):
    temp_v1 = velocities[i]
    temp_v2 = velocities[i+1]
    temp_t = time_between_in_seconds[i]
    a = acceleration(temp_v1, temp_v2, temp_t)
    accelerations.append(a)
    
np_accelerations = np.array(accelerations)
time_consecutive.pop()
...
#customizing plot
plt.suptitle('Acceleration vs. Time', fontsize=25, fontfamily='fantasy')
plt.ylabel('Acceleration (cm/s$^2$)', fontsize=14, fontfamily='sans-serif', weight="bold") #add and customize y axis label
